...
int main(int argc, char * argv[])
{
    ...
    syslog (LOG_NOTICE, "Program started updating procedure");
    ...
    string url_path_to_files = string(Configuration::getInstance()->get_update_url());
    ...
    if (curl) {
        ...
        bodyfile = fopen("./ja3.tar.gz", "wb"); //temporary file
        ...
        curl_easy_setopt(curl, CURLOPT_URL, url_path_to_files.c_str());
   ...
   ifstream ifs3("./whitelisting.json");
 ...
 if (!compare_fingerprints(items, old_items)) {
//   cout << "fingerprints are different" << endl;
   string com = "cp -f ./whitelisting.json ";
   com = com.append(fingerprints_file);
   system(com.c_str());
   syslog (LOG_INFO, "Jupdater: finished, new file loaded.");
   closelog ();
   return 2;
 }
...
//     cout << ii->getJa3_hash() << endl;
 //}
 syslog (LOG_INFO, "Jupdater: finished");
    closelog ();
    return 1;
  }
