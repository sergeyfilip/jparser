...
//  Preparing CURL to set elastic
    ...
    if (curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_URL, sz.c_str());
...
int main(int argc, char * argv[])
{
    ...
    try {
        ...
        while(std::getline (ifs2,name)) {
            ...
            readerl.parse(name, objl);
            const Json::Value& st = objl["desc"];
            const Json::Value& st1 = objl["Ja3_hash"];
            const Json::Value& st2 = objl["category"];
